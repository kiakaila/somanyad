...
exports.emailVerify = function (req, res) {
  ...
  EmailVerified.find({_id: key}, function (err, sendEmail) {
    ...
    EmailVerified.findOrCreate({email: email, user: sendEmail.user}, function (err, emailV) {
      if (err || emailV == null) {
        return res.send({
          status: "Failure",
...
exports.emailVerifyList = function (req, res) {
  var uid = req.user._id;
  EmailVerified.find({user: uid}, function (err, emails) {
    if (err || emails == null) {
      return res.send({
        status: "Failure",
...
exports.removeEmailVerify = function (req, res) {
  var email = req.query.email;
  var uid = req.user._id;
  EmailVerified.remove({email: email, user: uid}, function (err, email) {
    if (err || email == null) {
      return res.send({
        status: "Failure",
